#!/bin/bash
# Build ClaudeCode-Status menu bar app
# Usage: ./build.sh [--install] [--vm-name=NAME] [--port=PORT] [--app-name=NAME]
#
# Compiles ClaudeCodeStatus.swift into a standalone .app bundle.
...
LAUNCH_AGENT="com.claudecode.status${VM_NAME:+.${VM_NAME#claudecode-}}"
# For default "claudecode", launch agent is "com.claudecode.status" (no suffix)
if [ "$VM_NAME" = "claudecode" ]; then
...
SWIFT_SOURCE="$BUILD_DIR/ClaudeCodeStatus.swift"
sed \
  -e "s|private let vmName = \"claudecode\"|private let vmName = \"${VM_NAME}\"|g" \
  -e "s|private let portalURL = \"http://localhost:8080\"|private let portalURL = \"http://localhost:${PORTAL_PORT}\"|g" \
  -e "s|private let launchAgentLabel = \"com.claudecode.status\"|private let launchAgentLabel = \"${LAUNCH_AGENT}\"|g" \
  -e "s|title: \"Quit ClaudeCode-Status\"|title: \"Quit ${APP_NAME}\"|g" \
  "$SCRIPT_DIR/ClaudeCodeStatus.swift" > "$SWIFT_SOURCE"

# Compile from the generated source
swiftc \
  -O \
  -framework Cocoa \
  -o "$BUILD_DIR/$BUNDLE_NAME/Contents/MacOS/ClaudeCodeStatus" \
  "$SWIFT_SOURCE"

# Generate Info.plist with correct app name
sed \
  -e "s|ClaudeCode-Status|${APP_NAME}|g" \
  -e "s|com.claudecode.status|${LAUNCH_AGENT}|g" \
...
if [ "$INSTALL" = true ]; then
  ...
  pkill -f "ClaudeCodeStatus.*${VM_NAME}" 2>/dev/null || true
  sleep 0.5
  rm -rf "/Applications/$BUNDLE_NAME"
  # Clean up old name variants for default instance
  if [ "$VM_NAME" = "claudecode" ]; then
    rm -rf "/Applications/ClaudeCodeStatus.app"
  fi
