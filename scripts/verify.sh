#!/bin/bash
# Claude Code Lima -- End-State Verification
...
echo ""
echo -e "${BOLD}${CYAN}===============================================${NC}"
echo -e "${BOLD}  Claude Code Lima -- System Verification${NC}"
...
# ClaudeCode-Status app (instance-specific)
...
if [ -z "$VM_STATUS" ]; then
  ...
  if [ "$VM_STATUS" = "Running" ]; then
    ...
    VM_CHECK_SCRIPT='
      ...
      echo "BASHRC_ENV=$(grep -cF "# --- Claude Code environment" /home/claude/.bashrc 2>/dev/null || echo 0)"
      echo "ZSHRC_ENV=$(grep -cF "# --- Claude Code environment" /home/claude/.zshrc 2>/dev/null || echo 0)"
    ...
    [ "$(get_val BASHRC_ENV)" != "0" ] && passed ".bashrc Claude Code environment block" || failed ".bashrc Claude Code environment block"
    [ "$(get_val ZSHRC_ENV)" != "0" ] && passed ".zshrc Claude Code environment block" || failed ".zshrc Claude Code environment block"
