...
usage() {
  ...
  echo ""
  echo "Options:"
  echo "  --name=X            Target a named instance (default: claudecode)"
  echo ""
...
do_backup() {
  ...
  if [[ -d "$WORKSPACE_DIR" ]]; then
    ...
    cp -rc "$WORKSPACE_DIR" "$dest/claudecode-workspace"
  else
  ...
  echo "Done: Backup complete: $dest"
}
...
do_restore() {
  ...
  if [[ -d "$backup_path/claudecode-workspace" ]]; then
    if [[ -d "$WORKSPACE_DIR" ]]; then
      printf "! Workspace '%s' already exists. Overwrite? [y/N] " "$WORKSPACE_DIR"
      read -r confirm
      if [[ "$confirm" =~ ^[Yy]$ ]]; then
        rm -rf "$WORKSPACE_DIR"
        cp -rc "$backup_path/claudecode-workspace" "$WORKSPACE_DIR"
  ...
  echo "Done: Restore complete. Start your VM with: limactl start $vm_name"
}
