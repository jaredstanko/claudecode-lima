...
echo ""
echo -e "${BOLD}${RED}===============================================${NC}"
echo -e "${BOLD}  Claude Code Lima -- Host Cleanup${NC}"
if [ -n "$INSTANCE_SUFFIX" ]; then
  echo -e "${BOLD}  Instance: ${RED}${INSTANCE_NAME}${NC}"
fi
echo -e "${BOLD}${RED}===============================================${NC}"
echo ""
echo "  This will remove Claude Code Lima components from your Mac."
...
# --- 2. Remove ClaudeCode-Status menu bar app ---
...
if [ "$VM_NAME" = "claudecode" ]; then
  for old_app in "ClaudeCodeStatus.app"; do
...
# Desktop bookmark
BOOKMARK_PATH=$(cc_bookmark_path)
...
if [ -d "$WORKSPACE" ]; then
  ...
  echo ""
  echo "  This includes:"
  echo "    * claude-home/ -- Claude Code config, settings"
...
echo "    * This repo (claudecode-lima/)"
if [ -d "$WORKSPACE" ]; then
  echo "    * ${WORKSPACE}/ (you chose to keep it)"
fi
echo ""
echo "  To do a fresh install: ./install.sh${INSTANCE_SUFFIX:+ --name=${_CC_NAME}}"
echo ""
