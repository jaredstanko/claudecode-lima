#!/bin/bash
# Claude Code Lima -- Deterministic Host Installer for macOS
...
#
# Tools are installed at their latest versions. The Ubuntu VM image is pinned in claudecode.yaml.
# This script is idempotent -- safe to re-run if interrupted.
#
# Usage:
#   ./install.sh                        # Normal install (default "claudecode" instance)
#   ./install.sh --verbose              # Show full output from each step
#   ./install.sh --name=v2              # Parallel install as "claudecode-v2"
...
LOG_FILE="$SCRIPT_DIR/claudecode-install-$(date +%Y%m%dT%H%M%S).log"
...
echo ""
echo -e "${BOLD}${CYAN}===============================================${NC}"
echo -e "${BOLD}  Sandbox My AI -- Claude Code Lima Installer${NC}"
...
# Initialize log
echo "=== Claude Code Lima Install ($INSTANCE_NAME) $(date -u +%Y-%m-%dT%H:%M:%SZ) ===" > "$LOG_FILE"
...
# Generate instance-specific claudecode.yaml from template
GENERATED_YAML="$SCRIPT_DIR/.claudecode-${INSTANCE_NAME}.yaml"
# Workspace path needs ~ prefix for Lima yaml (not expanded $HOME)
WORKSPACE_TILDE="${WORKSPACE/#$HOME/~}"
sed \
  -e "s|~/claudecode-workspace/|${WORKSPACE_TILDE}/|g" \
  -e "s|set-hostname claudecode|set-hostname ${INSTANCE_NAME}|g" \
  -e "s|lima-claudecode|${INSTANCE_NAME}|g" \
  "$SCRIPT_DIR/claudecode.yaml" > "$GENERATED_YAML"
...
if [ -n "$VM_STATUS" ]; then
  ...
  echo "        Creating VM from claudecode.yaml (this takes 3-5 minutes)..."
...
step "Provisioning sandbox (installs Claude Code, tools)..."
...
echo ""
echo "  3. Click the icon again, then click \"New Claude Code Session\""
...
if [ -n "$INSTANCE_SUFFIX" ]; then
  echo ""
  echo "  All scripts support --name=${_CC_NAME} to target this instance:"
  echo "    ./scripts/launch.sh --name=${_CC_NAME}"
  echo "    ./scripts/upgrade.sh --name=${_CC_NAME}"
  echo "    ./scripts/verify.sh --name=${_CC_NAME}"
  echo "    ./scripts/uninstall.sh --name=${_CC_NAME}"
fi
echo ""
